...
Add `swift-google-monitoring-dashboard-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-monitoring-dashboard-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudMonitoringDashboardV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudMonitoringDashboardV1 <target-name> --package swift-google-monitoring-dashboard-v1
```
